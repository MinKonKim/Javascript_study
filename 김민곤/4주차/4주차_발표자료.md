...
> ### 생성자 함수 내부에서의 this
  ...
  + ```javascript
    ...
    */
    ...
    ``` 
   ===========================
   ## 명시적으로 this를 바인딩 하는 방법 Chapter 3 this
## 상황에 따라 달라지는 this
- this는 기본적으로 실행 컨텍스트가 생성될 때 함께 결정됩니다.
- this는 함수를 호출할 때 결정된다고 할 수 있다.
...
JavaScript에서 `this`는 함수 호출 방식에 따라 값이 결정됩니다. 그러나 때로는 `this`를 명시적으로 특정 객체에 바인딩해야 하는 경우가 있습니다. 이를 위해 JavaScript에서는 `bind()`, `call()`, `apply()` 메서드를 사용할 수 있습니다.
